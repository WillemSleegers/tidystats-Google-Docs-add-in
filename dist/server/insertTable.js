...
const splitParagraphAt = (pos) => {
  ...
  if (inParagraph) {
    ...
    if (offset == par.getNumChildren()) {
      ...
      return par
        .getParent()
        .insertParagraph(par.getParent().getChildIndex(par) + 1, "")
    }
    ...
    if (
      par == null ||
      (par.getType() != DocumentApp.ElementType.PARAGRAPH &&
        par.getType() != DocumentApp.ElementType.LIST_ITEM)
    ) {
...
const insertValue = (table, rownum, colnum, value, tag = null) => {
...
const insertTable = (name, groups) => {
  const cursor = doc.getCursor()
  const grps = JSON.parse(groups)

  // Make sure cursor is found
  // Make sure there are grps and that there are statistics
  if (!cursor) {
    DocumentApp.getUi().alert(
      "Could not insert table; the cursor should indicate the position."
    )
    ...
    for (let j = 0; j < grps.length; j++) {
      for (let i = 0; i < grps[j].statistics.length; i++) {
        let statistic = grps[j].statistics[i]
        if (
          Object.keys(statistic).includes("lower") &&
    ...
    const el =
      cursor.getOffset() == 0 ? cursor.getElement() : splitParagraphAt(cursor)
    const parentEl = el.getParent()
    const table = parentEl.insertTable(
      parentEl.getChildIndex(el),
